...
def make_policies(scenario: ScenarioConfig) -> dict:
    ...
    adaptive = AdaptiveEpochPolicy(
        ...
        min_epoch=settings.policy_min_epoch,
        max_epoch=max(settings.policy_max_epoch_multiplier * nominal, settings.policy_max_epoch_floor),
    ...
    return {
        "fixed-small": FixedEpochPolicy(
            ...
            min_epoch=settings.policy_min_epoch,
            max_epoch=settings.policy_max_epoch_multiplier * nominal,
        ),
        ...
        "fixed-large": FixedEpochPolicy(
            epoch_size=max(nominal * settings.policy_max_epoch_multiplier, settings.policy_min_epoch),
            min_epoch=settings.policy_min_epoch,
            max_epoch=settings.policy_max_epoch_multiplier * nominal,
        ),
        "adaptive": adaptive,
